...
if [ ! -e umls.db ]; then
	if [ ! -d "$1" ]; then
		echo "Provide the path to the UMLS install directory, which is named something like \"2019AA\" and contains a \"META\" directory, as first argument when invoking this script."
		echo
	...
	if [ ! -d "$1/META" ]; then
		...
		echo "Point this script to the directory named something like \"2019AA\"."
		exit 1
	fi
	...
	sqlite3 umls.db "CREATE TABLE descriptions AS SELECT CUI, LAT, SAB, TTY, LUI,STR FROM MRCONSO WHERE LAT = 'ENG'"
